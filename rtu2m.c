...
int main (int argc, char *argv[]) {
	...
	HParser *endSection = h_sequence(endBit, f1, fcnCode, len, NULL);
	...
	HParser *RTU2M = h_sequence(by, by, lengthCheck, lengthVal, flags, dataSections, h_end_p(), NULL);
	char buf[100] = "\xff\xff\x08\xff\x71\x02\x01\x01\x81\x01\x01\xce\xce";
	HParseResult *result = h_parse(RTU2M, buf, strnlen(buf, 100));
	if(result) {
		printf("yay\n");
	}
	else{
